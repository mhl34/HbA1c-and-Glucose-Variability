...
class Conv1DGeneticModel(nn.Module):
    def __init__(self, num_features, dropout_p = 0, normalize = False, seq_len = 28):
        ...
        # output: 12 x 8
        self.conv1 = nn.Conv1d(in_channels = self.num_features + 1, out_channels = 28, kernel_size = 5, stride = 2)
        # input: 12 x 8
        # output: 10 x 16
        self.conv2 = nn.Conv1d(in_channels = 28, out_channels = 48, kernel_size = 3, stride = 1)
        # input: 10 x 16
        # output: 6 x 64
        self.conv3 = nn.Conv1d(in_channels = 48, out_channels = 84, kernel_size = 5, stride = 1)
        self.dropout = nn.Dropout(dropout_p)
        self.fc1 = nn.Linear(84 * (((self.seq_len - 4) // 2) - 2 - 4), 64)
